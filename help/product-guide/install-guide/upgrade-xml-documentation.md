...
| 預期回應 |  — 如果可以移動所有必要的節點，您將會獲得通過檢查。 <br> — 如果目標位置出現節點，您會看到相關錯誤。 清理存放庫\（刪除節點/var/dxml\）並重新安裝升級套件，然後再次觸發此端點。 <br>**注意：** 這不是常見的錯誤，因為《3.xExperience Manager指南》之前並未使用目標位置。 <br>  — 如果此指令碼失敗，請勿繼續並報告給您的客戶成功團隊。 |
...
1. 從下載4.3.1版套件 [Adobe軟體發佈入口網站](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html).
1. 安裝4.3.1版本的套件。
1. 您可以選擇點選觸發器，以啟動翻譯對應升級工作。 如需詳細資訊，請參閱 [透過Servlet啟用指令碼的觸發](#enable-trigger-serverlet-4-3-1).
