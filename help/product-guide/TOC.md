...
- 安裝和設定 {#install-guide}
   - 內部部署 {#on-prem-ig}
      ...
      - 下載、安裝和升級 {#download-install-upgrade-aemg}
         - {hide-from-toc}[下載與安裝概述](./install-guide/download-install.md)
         - {hide-from-toc}[技術需求](./install-guide/download-install-technical-requirements.md)
         - {hide-from-toc}[安裝Adobe Experience Manager](./install-guide/download-install-aem.md)
         - {hide-from-toc}[第一次下載並安裝AEM Guides](./install-guide/download-install-aemg-first-time.md)
         - {hide-from-toc}[設定Dispatcher](./install-guide/download-install-configure-dispatcher.md)
         - {hide-from-toc}[驗證AEM Guides安裝](./install-guide/download-install-verify-aemg-installation.md)
         - {hide-from-toc}[升級AEM Guides](./install-guide/upgrade-xml-documentation.md)
         - {hide-from-toc}[解除安裝AEM Guides](./install-guide/download-install-unistall-aemg.md)
         - {hide-from-toc}[效能最佳化建議](./install-guide/download-install-recommend-perf-optimiz.md)
      - 使用使用者群組及安全性 {#user-group-sec}
         - {hide-from-toc}[使用者管理與安全性](./install-guide/user-admin-sec.md)
      - 內容移轉 {#content-migration}
         - {hide-from-toc}[非UUID移轉至UUID內容](./install-guide/migrate-uuid-non-uuid.md)
         - 移轉程式 {#migration-process}
            - {hide-from-toc}[4.3.1非UUID移轉至4.3.2 UUID內容移轉](./install-guide/migrate-non-uuid-4-3.md)
            - {hide-from-toc}[4.6.0非UUID移轉至4.6.1 UUID內容移轉](./install-guide/migrate-non-uuid-uuid-4-6.md)
         - {hide-from-toc}[移轉現有內容](./install-guide/migrate-content.md)
         - {hide-from-toc}[上傳現有的DITA內容](./install-guide/migrate-content-upload-existing-dita-content.md)
         - {hide-from-toc}[移轉非DITA內容](./install-guide/migrate-content-non-dita.md)
      - 使用自訂DITA-OT和專業化 {#custom-dita-ot-spec}
         - {hide-from-toc}[使用自訂DITA-OT與DITA專業化](./install-guide/dita-ot-specialization.md)
      - {hide-from-toc}[設定資料夾的後處理](./install-guide/conf-folder-post-processing.md)
      - {hide-from-toc}[設定檔案名稱](./install-guide/conf-file-names.md)
      - {hide-from-toc}[根據UUID設定自動檔案名稱](./install-guide/conf-auto-uuid-filenames.md)
      - {hide-from-toc}[設定有效檔案名稱字元的Regx](./install-guide/conf-file-names-valid-regx.md)
      - {hide-from-toc}[為AEM網站輸出設定有效的檔案名稱](./install-guide/conf-file-names-valid-regx-aem-site-output.md)
      - 使用主題和地圖範本 {#topic-map-template}
         - {hide-from-toc}[設定主題與對應範本](./install-guide/conf-template-tags.md)
         - {hide-from-toc}[設定自訂DITA主題範本](./install-guide/conf-template-tags-custom-dita-topic-template.md)
         - {hide-from-toc}[設定自訂DITA map範本](./install-guide/conf-template-tags-custom-dita-map-templates.md)
      - 使用檔案狀態 {#doc-state}
         - {hide-from-toc}[設定檔案狀態](./install-guide/customize-doc-state.md)
      - 編輯器設定 {#editor-configs}
         - {hide-from-toc}[設定編輯器設定](./install-guide/editor-settings.md)
      - 使用網頁編輯器設定 {#web-editor-configs}
         - {hide-from-toc}[自訂Web編輯器](./install-guide/conf-web-editor.md)
         - {hide-from-toc}[自訂工具列](./install-guide/conf-web-editor-customize-toolbar.md)
         - {hide-from-toc}[設定標籤檢視的預設值](./install-guide/configure-default-value-tags-view.md)
         - {hide-from-toc}[設定簽入和簽出圖示的標題](./install-guide/conf-checkin-checkout-title.md)
         - {hide-from-toc}[設定UUID連結的顯示](./install-guide/conf-uuid-based-links.md)
         - {hide-from-toc}[在同一個索引標籤中開啟DITA主題或對應檔案](./install-guide/open-dita-files-same-tab.md)
         - {hide-from-toc}[在左側面板中設定自訂面板](./install-guide/configure-custom-panel.md)
         - {hide-from-toc}[在網頁編輯器中設定檔案自動儲存](./install-guide/auto-save-in-editor.md)
         - {hide-from-toc}[設定檔案瀏覽對話方塊的篩選器](./install-guide/conf-custom-file-filters.md)
         - {hide-from-toc}[設定關閉時儲存為新版本的提示](./install-guide/conf-save-as-new-version-close.md)
         - {hide-from-toc}[設定關閉時簽入檔案的提示](./install-guide/conf-checkin-file-close.md)
         - {hide-from-toc}[設定允許的特殊字元](./install-guide/conf-special-chars.md)
         - {hide-from-toc}[自動產生專案ID](./install-guide/auto-generate-ids.md)
         - {hide-from-toc}[自訂AEM的預設字典](./install-guide/customize-aem-custom-dictionary.md)
         - {hide-from-toc}[設定文字篩選器](./install-guide/config-text-filters.md)
         - {hide-from-toc}[設定查詢的LimitReads數目](./install-guide/conf-query-limitreads.md)
         - {hide-from-toc}[安裝以文章為基礎的發行套件](./install-guide/configure-article-based-publishing.md)
         - {hide-from-toc}[從使用者介面設定資料來源聯結器](./install-guide/conf-data-source-connector-tools.md)
         - {hide-from-toc}[將進階地圖編輯器設定為預設值](./install-guide/conf-map-editor.md)
         - {hide-from-toc}[預設包含@navtitle屬性](./install-guide/auto-add-navtitle.md)
         - {hide-from-toc}[設定選項以在Oxyor中編輯](./cs-install-guide/conf-edit-in-oxygen.md)
         - {hide-from-toc}[設定其他特殊字元](./web-editor/configure-additional-special-characters.md)
      - 使用全域和檔案夾層級的設定檔 {#global-folder-profiles}
         - {hide-from-toc}[設定全域或資料夾層級的設定檔](./install-guide/conf-folder-level.md)
      - 設定版本 {#version-mgt}
         - {hide-from-toc}[版本管理](./install-guide/version-management.md)
      - 使用桌上型XML編輯器 {#desktop-xml-editor}
         - {hide-from-toc}[整合案頭式XML編輯器](./install-guide/integrate-desktop-editors.md)
      - 使用輸出產生設定 {#output-gen-config}
         ...
         - 原生PDF發佈 {#config-native-pdf-publish}
            ...
            - 常見內容樣式 {#content-styles}
               - {hide-from-toc}[使用一般內容樣式](./native-pdf/stylesheet.md)
               - {hide-from-toc}[使用自訂變更列樣式](./native-pdf/change-bar-style.md)
               - {hide-from-toc}[在目錄專案與主題內容上套用自訂樣式](./native-pdf/custom-style-toc.md)
               - {hide-from-toc}[在PDF輸出中新增自訂書籤](./native-pdf/add-custom-bookmark.md)
               - {hide-from-toc}[使用JavaScript處理內容或樣式](./native-pdf/use-javascript-content-style.md)
               - {hide-from-toc}[套用樣式至註腳](./native-pdf/footnote-number-style.md)
         - {hide-from-toc}[為原生PDF發佈設定JVM旗標](./native-pdf/configure-jvm-flags.md)
         - {hide-from-toc}[設定發佈輸出的基底輸出位置](./native-pdf/configure-base-output-location.md)
         - {hide-from-toc}[設定和自訂工作流程](./install-guide/customize-workflows.md)
      - 使用翻譯設定 {#translate-config}
         - {hide-from-toc}[翻譯內容](./install-guide/translation.md)
      - 使用AEM Assets搜尋 {#aem-asset-search}
         - {hide-from-toc}[設定AEM Assets UI的搜尋](./install-guide/conf-dita-search.md)
      - 使用資產處理 {#asset-processing-on-prem}
         - {hide-from-toc}[設定資產處理功能](./install-guide/configure-asset-processing.md)
      - 使用B樹清理{#btree-cleanup}
         - {hide-from-toc}[設定B樹狀結構清理](./install-guide/configure-btree-clean-up-job.md)
      - 自訂索引{#custom-indexing}
         - {hide-from-toc}[部署自訂索引以尋找和取代（Source檢視）](./install-guide/custom-indexing-prem.md)
      - 使用DITA資產復寫{#dita-assets-replication}
         - {hide-from-toc}[設定DITA資產復寫](./install-guide/configure-dita-asset-replication.md)
      - {hide-from-toc}[附錄](./install-guide/appendix.md)
   - 雲端服務 {#cs-ig}
      - {hide-from-toc}[關於本指南](./cs-install-guide/introduction.md)
      - 部署和設定AEM Guides as Cloud Service {#deploy-conf-aemg-cs}
         - {hide-from-toc}[下載並安裝](./cs-install-guide/download-install.md)
         - {hide-from-toc}[部署AEM Guides模組](./cs-install-guide/download-install-dxml-first-time.md)
         - {hide-from-toc}[設定Dispatcher](./cs-install-guide/download-install-configure-dispatcher.md)
         - {hide-from-toc}[驗證AEM Guides安裝](./cs-install-guide/download-install-verify-dxml-installation.md)
         - {hide-from-toc}[升級AEM Guides](./cs-install-guide/download-install-upgrade-dxml.md)
         - {hide-from-toc}[升級AEM Guides as Cloud Service的其他設定](./cs-install-guide/additional-config-for-cloud-service.md)
         - {hide-from-toc}[設定覆寫](./cs-install-guide/download-install-additional-config-override.md)
         - {hide-from-toc}[效能最佳化建議](./cs-install-guide/download-install-recommend-perf-optimiz.md)
      - 使用使用者群組及安全性 {#user-group-sec-cs}
         - {hide-from-toc}[使用者管理與安全性](./cs-install-guide/user-admin-sec.md)
      - 內容移轉 {#migrate-con-cs}
         - {hide-from-toc}[移轉現有內容](./cs-install-guide/migrate-content.md)
         - {hide-from-toc}[上傳現有的DITA內容](./cs-install-guide/migrate-content-upload-existing-dita-content.md)
         - {hide-from-toc}[移轉非DITA內容](./cs-install-guide/migrate-content-non-dita.md)
         - {hide-from-toc}[將內容從內部部署移轉至Cloud Service](./cs-install-guide/migrate-on-premise-content-cloud.md)
      - 使用自訂DITA-OT和專業化 {#custom-dita-ot-spec-cs}
         - {hide-from-toc}[使用自訂DITA-OT與DITA專業化](./cs-install-guide/dita-ot-specialization.md)
      - 設定UUID和非UUID型檔案名稱 {#conf-non-uuid-filename-cs}
         - {hide-from-toc}[設定資料夾的後處理](./cs-install-guide/conf-folder-post-processing.md)
         - {hide-from-toc}[設定檔案名稱](./cs-install-guide/conf-file-names.md)
         - {hide-from-toc}[根據UUID設定自動檔案名稱](./cs-install-guide/conf-auto-uuid-filenames.md)
         - {hide-from-toc}[設定有效檔案名稱字元的Regx](./cs-install-guide/conf-file-names-valid-regx.md)
         - {hide-from-toc}[為AEM網站輸出設定有效的檔案名稱](./cs-install-guide/conf-file-names-valid-regx-aem-site-output.md)
      - 使用主題和地圖範本 {#topic-map-template-cs}
         - {hide-from-toc}[設定主題與對應範本](./cs-install-guide/conf-template-tags.md)
         - {hide-from-toc}[設定自訂DITA主題範本](./cs-install-guide/conf-template-tags-custom-dita-topic-template.md)
         - {hide-from-toc}[設定自訂DITA map範本](./cs-install-guide/conf-template-tags-custom-dita-map-templates.md)
      - 使用檔案狀態 {#doc-state-cs}
         - {hide-from-toc}[設定檔案狀態](./cs-install-guide/customize-doc-state.md)
         - {hide-from-toc}[設定檔案狀態篩選器](./cs-install-guide/config-doc-state-filters.md)
      ...
      - 使用網頁編輯器設定 {#web-editor-configs-cs}
         - {hide-from-toc}[自訂Web編輯器](./cs-install-guide/conf-web-editor.md)
         - {hide-from-toc}[自訂工具列](./cs-install-guide/conf-web-editor-customize-toolbar.md)
         - {hide-from-toc}[設定AI支援的智慧型建議以進行編寫](./cs-install-guide/conf-smart-suggestions.md)
         - {hide-from-toc}[設定AI支援的Smart Help以搜尋內容](./cs-install-guide/conf-smart-help.md)
         - {hide-from-toc}[設定標籤檢視的預設值](./cs-install-guide/configure-default-value-tags-view.md)
         - {hide-from-toc}[設定UUID連結的顯示](./cs-install-guide/conf-uuid-based-links.md)
         - {hide-from-toc}[設定貼上表格的顯示](./cs-install-guide/conf-pasted-tables.md)
         - {hide-from-toc}[在同一個索引標籤中開啟DITA主題或對應檔案](./cs-install-guide/open-dita-files-same-tab.md)
         - {hide-from-toc}[在左側面板中設定自訂面板](./cs-install-guide/configure-custom-panel.md)
         - {hide-from-toc}[在網頁編輯器中設定檔案自動儲存](./cs-install-guide/auto-save-in-editor.md)
         - {hide-from-toc}[設定關閉時儲存為新版本的提示](./cs-install-guide/conf-save-as-new-version-close.md)
         - {hide-from-toc}[設定關閉時簽入檔案的提示](./cs-install-guide/conf-checkin-file-close.md)
         - {hide-from-toc}[設定允許的特殊字元](./cs-install-guide/conf-special-chars.md)
         - {hide-from-toc}[自動產生專案ID](./cs-install-guide/auto-generate-ids.md)
         - {hide-from-toc}[自訂AEM的預設字典](./cs-install-guide/customize-aem-custom-dictionary.md)
         - {hide-from-toc}[設定文字篩選器](./cs-install-guide/config-text-filters.md)
         - {hide-from-toc}[設定檔案瀏覽對話方塊的篩選器](./cs-install-guide/conf-custom-file-filters.md)
         - {hide-from-toc}[安裝以文章為基礎的發行套件](./cs-install-guide/configure-article-based-publishing.md)
         - {hide-from-toc}[設定單一主題PDF產生](./cs-install-guide/conf-pdf-generation-dita-ot.md)
         - {hide-from-toc}[在網頁編輯器中設定翻譯功能](./cs-install-guide/conf-translation-web-editor.md)
         - {hide-from-toc}[設定資料來源聯結器](./cs-install-guide/conf-data-source-connector.md)
         - {hide-from-toc}[從使用者介面設定資料來源聯結器](./cs-install-guide/conf-data-source-connector-tools.md)
         - {hide-from-toc}[將進階地圖編輯器設定為預設值](./cs-install-guide/conf-map-editor.md)
         - {hide-from-toc}[預設包含@navtitle屬性](./cs-install-guide/auto-add-navtitle.md)
      - 使用全域和檔案夾層級的設定檔 {#global-folder-profiles-cs}
         - {hide-from-toc}[設定全域或資料夾層級的設定檔](./cs-install-guide/conf-folder-level.md)
      - 設定版本 {#version-mgt-cs}
         - {hide-from-toc}[版本管理](./cs-install-guide/version-management.md)
      - 使用桌上型XML編輯器 {#desktop-xml-editor}
         - {hide-from-toc}[整合案頭式XML編輯器](./cs-install-guide/integrate-desktop-editors-cs.md)
      - 設定Workfront {#configure-workfront-cs}
         - {hide-from-toc}[設定Workfront](./cs-install-guide/conf-workfront.md)
      - 使用輸出產生設定 {#output-gen-config-cs}
         ...
         - {hide-from-toc}[建立主題和體驗片段之間的對應](./cs-install-guide/conf-experience-fragment-mapping-cs.md)
         - {hide-from-toc}[設定發佈輸出的基底輸出位置](./native-pdf/configure-base-location-cs.md)
      - 使用自訂工作流程 {#custom-workflow-cs}
         - {hide-from-toc}[設定和自訂工作流程](./cs-install-guide/customize-workflows.md)
      - 使用翻譯設定 {#translate-config-cs}
         - {hide-from-toc}[翻譯內容](./cs-install-guide/translation.md)
      - 使用AEM Assets搜尋 {#aem-asset-search-cs}
         - {hide-from-toc}[設定AEM Assets UI的搜尋](./cs-install-guide/conf-dita-search.md)
      - 使用資產處理 {#asset-processing-cloud}
         - {hide-from-toc}[設定資產處理功能](./cs-install-guide/configure-asset-processing-cs.md)
      - 使用B樹清理 {#btree-cleanup-cs}
         - {hide-from-toc}[設定B樹狀結構清理](./cs-install-guide/configure-btree-cleanup-cs.md)
      - 自訂索引 {#custom-indexing-cs}
         - {hide-from-toc}[部署自訂索引以尋找和取代（Source檢視）](./cs-install-guide/custom-indexing.md)
      - 使用DITA資產復寫 {#dita-assets-replication-cs}
         - {hide-from-toc}[設定DITA資產復寫](./cs-install-guide/configure-dita-assets-replication.md)
      - {hide-from-toc}[附錄](./cs-install-guide/appendix.md)
