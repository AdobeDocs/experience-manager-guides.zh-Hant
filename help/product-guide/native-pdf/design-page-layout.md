...
除了預先定義的欄位之外，您也可以在頁面版面配置中將中繼資料資訊新增為變數或欄位。 此中繼資料儲存在您的來源DITA中 **對應內容**，或可從DITA中選取 **對應檔案屬性** 或 **主題檔案屬性** 並輕鬆插入頁面版面配置中。

您可以從下列選項中選取中繼資料：

* **對應內容** 包含您在中定義的中繼資料 `<topicmeta>` DITA map的元素。
* **對應檔案屬性** 包括中繼資料，您可從中存取 **屬性** DITA map的頁面。
* **主題檔案屬性** 包括中繼資料，您可從中存取 **屬性** 主題頁面。


您可以合併中繼資料來源 **對應檔案屬性** 和 **主題檔案屬性** 在單一檔案中。 例如，您可以發佈一個PDF，其封面頁上的地圖示題和其他頁面標題中的主題標題。 若要這麼做，您可以從以下位置新增地圖示題中繼資料： **對應檔案屬性** 至封面版面配置。 然後，從新增主題標題中繼資料 **主題檔案屬性**  到「章節和主題」頁面配置的頁首。
