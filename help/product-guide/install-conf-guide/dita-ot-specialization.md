...
下列標籤會根據您的Experience Manager Guides設定，提供使用自訂DITA-OT外掛程式的指示： Cloud Service或內部部署。
...
>[!NOTE]
>
> 您可以將自訂DITA設定檔匯出為套件，並上傳至其他AEM Guides執行個體以節省時間。 如需詳細資訊，請參閱[附錄](./aemg-appendix.md)。
