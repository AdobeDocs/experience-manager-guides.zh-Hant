...
### 將您現有的Word檔案轉換為DITA主題型別檔案

下列標籤會根據您的Experience Manager Guides設定，提供將現有Word檔案轉換為DITA主題型別檔案的指示： Cloud Service或內部部署。
...
1. 將來源Word檔案上傳至此資料夾。 如需在DAM上傳內容的資訊，請參閱[上傳現有DITA內容](upload-dita-content.md#)。
...
如果您是AEM Guides as a Cloud Service的現有使用者，並從2021年8月版本升級至2022年1月或以上版本，請更新指定的屬性，因為只有少數檔案已移動。
...
大致上，轉換程式需要您[準備InDesign檔案以進行轉換](aemg-appendix.md#id195DBF0045Z)和[準備對應檔案以便InDesign移轉至DITA](aemg-appendix.md#id194AF0003HT)，然後您需要遵循指定的程式來執行轉換程式。
...
1. 在`apps`節點內建立`config`資料夾的覆蓋節點。
   ...
   - 在`inputDir`元素中，指定可使用來源InDesign檔案的輸入資料夾位置。 例如，如果您的InDesign檔案儲存在`projects`資料夾中名為`indesigntodita`的資料夾中，則指定位置為： `/content/dam/idmlfiles/indesigntodita/`
   ...
   - 在`mapStyle`元素中，指定對應檔案的位置，該對應檔案包含InDesign檔案樣式到DITA元素的對應。 預設對應儲存在檔案中，檔案位於：
     ...
     >[!NOTE]
     >
     > 如需`stmap.adobeidml.xml`檔案結構以及如何自訂的詳細資訊，請參閱[附錄](aemg-appendix.md#id194AF0003HT)。
...
1. 將來源InDesign檔案上傳至此資料夾。 如需在DAM上傳內容的資訊，請參閱[上傳現有DITA內容](upload-dita-content.md#)。
...
### 將現有的XHTML檔案轉換為DITA主題

下列標籤會根據您的Experience Manager Guides設定，提供將現有XHTML檔案轉換為DITA主題的指示： Cloud Service或內部部署。
...
1. 將來源XHTML檔案上傳至此資料夾。 如需在DAM上傳內容的資訊，請參閱[上傳現有DITA內容](upload-dita-content.md#)。
...
轉換後，結構化內容可移轉至AEM Guides。 如需詳細資訊，請檢視[使用WebDAV工具和FrameMaker上傳現有DITA內容](./upload-dita-content.md)。

## 移轉任何其他結構化檔案 {#id1949B0590YK}

AEM Guides可讓您將現有的結構化檔案轉換為有效的DITA檔案。 您需要指定輸入和輸出資料夾位置、轉換檔案的位置、儲存最終輸出的副檔名，以及是否需要檔案的新版本。

下列標籤會提供根據您的Experience Manager Guides設定移轉任何其他結構化檔案的指示：Cloud Service或內部部署。
...
1. 將來源結構化檔案上傳至此資料夾。 如需在DAM上傳內容的資訊，請參閱[上傳現有DITA內容](upload-dita-content.md#)。
