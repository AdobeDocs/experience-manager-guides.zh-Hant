...
| --- | --- |
| 請求型別 | **取得** <br> **注意**：您可以使用網頁瀏覽器，以管理員身分登入AEM執行個體。 |
...
2. 已關閉所有翻譯任務。
3. 將`com.adobe.fmdita.translationservices.TranslationMapUpgradeScript`的記錄層級設定為`INFO`，並記錄至新的記錄檔（例如，`logs/translation_upgrade.log`）。
...
1. 從[Adobe軟體發佈入口網站](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html)下載&#x200B;**4.2**&#x200B;套件。
2. 安裝4.2套件。
3. 安裝後，請等待記錄檔中的下列訊息：
...
>[!TIP]
>
> 建議在版本&#x200B;**4.2.1**&#x200B;之上安裝&#x200B;**Hotfix4.2.1.3**。
...
2. 關閉所有翻譯任務。
3. 將`com.adobe.fmdita.translationservices.TranslationMapUpgradeScript`的記錄層級設定為`INFO`，並記錄至新檔案（例如，`logs/translation_upgrade.log`）。
...
1. 從[Adobe軟體發佈入口網站](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html)下載&#x200B;**4.2.1**&#x200B;套件。
2. 安裝4.2.1套件。
...
如果您使用版本&#x200B;**4.3.1**，可以直接升級至&#x200B;**4.3.1.5**。

### 安裝版本4.3.1.5

1. 從[Adobe軟體發佈入口網站](https://experience.adobe.com/#/downloads/content/software-distribution/en/aem.html)下載&#x200B;**4.3.1.5**&#x200B;套件。
2. 安裝4.3.1.5封裝。
3. 請等候安裝程式順利完成。
4. 繼續升級自訂，如下一節所述。
...
如果您使用： **4.3.1**、**4.3.0**、**4.2**&#x200B;或&#x200B;**4.2.1 (Hotfix 4.2.1.3)**，可以直接升級至&#x200B;**4.4.0**。
...
5. 如果元件不存在，請插入它：
 ...
 — 描述： `DXML post process initiator step which will trigger a sling job for DXML post-processing of the modified/created asset`
      **處理程式標籤**
 — 程式：選取 `DXML Post Process Initiator`
 — 選取 `Handler Advance`
 — 選取`Done`
...
1. 已針對&#x200B;**DAM更新資產工作流程**&#x200B;建立「*節點*」的啟動器 — 對於條件`"jcr:content/jcr:mimeType!=video"`，「萬用字元」值應為：
   ...
   - 已針對&#x200B;**DAM更新資產工作流程 —** （條件&quot;`jcr:content/jcr:mimeType!=video`&quot;）修改&#x200B;*節點*&#x200B;的啟動器，「萬用字元」值應為：
...
1. 在`damAssetLucene` Oak索引中，在`/oak:index/damAssetLucene/indexRules/dam:Asset`中新增布林值屬性`indexNodeName=true`。
2. 在`/oak:index/damAssetLucene/indexRules/dam:Asset/properties`下新增名為`excerpt`的新節點，並設定如來源中所示的屬性：
   - `name=rep:excerpt`
   - `propertyIndex=true`
   - `notNullCheckEnabled=true`
3. 透過設定`reindex=true`重新索引`damAssetLucene`並等候它再次變成`false` （可能需要數小時）。
...
1. （選用）為大型存放庫增加Oak queryLimitReads
   ...
   如果有超過&#x200B;**100,000個DITA檔案**，請將`org.apache.jackrabbit.oak.query.QueryEngineSettingsService`下的`queryLimitReads`更新為大於資產數的值（範例： `200000`）、重新部署並繼續。
...
- 使用大於50 **的`order`值**，因為參考線使用`order=50`。
