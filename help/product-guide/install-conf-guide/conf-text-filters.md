...
AEM Guides提供在AEM存放庫的所選路徑中存在的檔案中搜尋文字的功能。 您可以使用篩選器搜尋來搜尋存放庫面板中的檔案或瀏覽檔案。 在Web編輯器中工作時，您需要使用檔案瀏覽對話方塊來插入影像、參照或關鍵參照等元素。
...
下列索引標籤提供根據Experience Manager Guides設定來設定文字篩選器的指示： Cloud Service或內部部署。
...
1. 按一下「**下載**」圖示，在本機系統上下載ui\_config.json檔案。 您可以接著對檔案進行變更，然後上傳相同的檔案。
   1. 在檔案中設定篩選器。 您也可以新增自訂篩選器，如下列範例所示：
      ...
      ****屬性****：要符合檔案中繼資料的屬性。 例如，若要僅允許屬性中有dita\_class中繼資料的檔案，屬性篩選器會將「jcr:content/metadata/dita\_class」作為其值。
