...
下列標籤會提供根據您的Experience Manager Guides設定，在搜尋序列化檔案中新增自訂DITA元素或屬性的指示： Cloud Service或On-Premise。
...
下列標籤會提供根據您的Experience Manager Guides設定，在`configMgr`中啟用中繼資料擷取選項的指示： Cloud Service或內部部署。

>[!BEGINTABS]

>[!TAB Cloud Service]

使用[組態覆寫](download-install-config-override.md#)中提供的指示來建立組態檔。 在設定檔案中，提供下列\(property\)詳細資訊，以設定中繼資料擷取選項：
...
下列標籤會提供指示，讓您根據您的Experience Manager Guides設定，從搜尋中排除暫時翻譯資料夾： Cloud Service或內部部署。
