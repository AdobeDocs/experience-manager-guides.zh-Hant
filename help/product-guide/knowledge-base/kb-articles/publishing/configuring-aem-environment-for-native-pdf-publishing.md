...
AEM Guides內含原生PDF發佈引擎，可讓使用者設計、開發及發佈PDF格式的內容。

它提供建立不同頁面配置、CSS範本的功能，並設計PDF範本與頁面配置和CSS。

在AEM Guides中設定此原生PDF的步驟因作業系統而異。 根據安裝AEM的作業系統，使用下列設定步驟。
...
原生PDF發佈引擎需要Oracle JDK在AEM crx-quickstart資料夾中產生節點模組。 預設支援下列作業系統：
...
4. 根據安裝的Java版本，使用以下命令設定JAVA_HOME變數
   ...
   1. 匯出JAVA\_HOME=/usr/lib/jvm/java-11.0.15.1
   2. 匯出路徑=$PATH： $JAVA\_HOME/bin
   3. 匯出LD\_LIBRARY\_PATH=/usr/lib/jvm/jdk-11.0.15.1/lib/server：/usr/java/jdk-11.0.15.1/lib/server
...
6. 將本文底部附加的&quot;_node_ modules.zip_&quot;複製到crx-quickstart/profiles/nodejs—b1aad0a7-9079-e56c-1ed8-6fcababe8166/目錄。
...
1. 安裝Oracle JAVA 11或Oracle JAVA 8。
...
4. 根據安裝的Java版本，使用以下命令設定JAVA_HOME變數
   ...
   匯出JAVA\_HOME= /Library/Java/JavaVirtualMachines/jdk-11.0.15.1.jdk/Contents/Home
...
8. 使用以下命令確認是否已安裝Java
   ...
   i)從/crx-quickstart/profiles/nodejs—b1aad0a7-9079-e56c-1ed8-6fcababe8166資料夾執行&#x200B;**./node-darwin/bin/node**&#x200B;命令
