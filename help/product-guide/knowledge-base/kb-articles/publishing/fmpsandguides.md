---
title: 使用AEM Guides中的FrameMaker Publishing Server (FMPS)進行發佈
...
# 使用AEM Guides中的FrameMaker Publishing Server (FMPS)進行發佈
...
使用FMPS，您可以自動發佈DITA和FM Assets的&#x200B;**PDF、Responsive HTML5**&#x200B;和&#x200B;**Epub**。

從「使用&#39;產生PDF&#39;功能表，選擇FrameMaker Publishing Server。

使用者可以提供「設定File(.sts)」和「ditaval」。 根據您提供的條件，使用雙精度篩選器完成篩選。

- **設定檔**：包含發佈時要FMPS遵循的所有設定的FrameMaker /FMPS發佈設定檔。 例如，使用自訂範本建立輸出、建立「標籤和出血」(PDF)以及使用TOC建立PDF。
...
>如果您使用UUID，請確定您已核取FrameMaker編輯偏好設定中的「使用UUID型參考」，並反之針對非UUID的AEM指南。
