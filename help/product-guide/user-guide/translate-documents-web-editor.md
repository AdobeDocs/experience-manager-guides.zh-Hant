---
title: 從網頁編輯器翻譯檔案
description: 從網頁編輯器將您的內容翻譯成多種語言。 瞭解如何在AEM Guides中建立翻譯專案、新增規則、檢視版本和關閉不同步的檔案。
...
1. 在 **存放庫** 面板，在地圖檢視中開啟DITA map檔案。
1. 按一下 **管理** 標籤。 此 **翻譯面板** 顯示可用的語言群組。
...
1. 您可以選取任何語言群組作為翻譯的目標。 如果您 **全選**，則選取的檔案會翻譯成現有語言群組中的所有可用語言。
...
1. 從 **其他語言**，您可以選擇已建立目標資料夾的任何語言，但不是任何語言群組的一部分。
...
>[!NOTE]
> 
>此功能適用於您使用Experience Manager指南2404版本或更新版本建立的新翻譯專案。  這不會影響任何現有的專案。

您的管理員可以設定 **翻譯專案完成後的清理** 下的選項 **翻譯** 定位於 **編輯器設定** 以自動停用或刪除翻譯專案。

為達到檔案管理效果，「Experience Manager指南」提供完成翻譯後刪除翻譯專案的功能。
