...
1. 選取&#x200B;**套用**。 隨即顯示包含主題和相關資產詳細資訊的清單。
   ...
   >[!NOTE]
   >
   > 當使用DITAVAL和Markdown檔案參照翻譯地圖時，以及當翻譯以工作復本為基礎時，如果參照存在於來源語言資料夾中，則會包含影像和其他連結的資產。 這些參照會顯示在「平移圖示板」的參照清單中，可讓您明確選取要平移的參照。 在翻譯期間，參照的資產會複製到目標語言資料夾，並以標準影像翻譯行為一致地處理。
...
新翻譯專案會以選取的主題版本建立。 此時，會顯示快顯訊息，確認已建立翻譯專案。 一旦所有目標語言副本在翻譯專案中可用，您會在收件匣中收到通知。 一旦翻譯專案中有目標語言副本可用，您就可以開始翻譯工作。 如需詳細資料檢視，請[開始翻譯工作](translation-first-time.md#id225IK030OE8)。
...
若要在目標復本中新增來源版本標籤，您的系統管理員必須在&#x200B;**Workspace設定** （針對&#x200B;**內部部署**）顯示為&#x200B;**設定**)中的&#x200B;**翻譯**&#x200B;索引標籤下，啟用&#x200B;**將來源版本標籤傳播到目標版本**&#x200B;選項。
...
1. 選取&#x200B;**強制同步**。 它會針對選取的「不同步」資產，將狀態設定為「同步」。
...
您的管理員可以在&#x200B;**Workspace設定** （針對&#x200B;**內部部署**&#x200B;顯示為&#x200B;**設定**）中的&#x200B;**翻譯**&#x200B;索引標籤下，設定&#x200B;**完成後翻譯專案清理**&#x200B;選項，以自動停用或刪除翻譯專案。
