...
功能表下拉式清單提供各種編輯動作的存取權，如下所示：

>[!BEGINTABS]

>[!TAB 新編輯器]

編輯器工具列的![功能表下拉式清單](images/editor-toolbar-menu-dropdown-new-editor-2-0.png){width="350"}


>[!TAB 舊編輯器]

編輯器工具列的![功能表下拉式清單](images/editor-toolbar-menu-dropdown-new.png){width="350"}
...
>[!NOTE]
>
> 在新編輯器中，「功能表」下拉式清單中不再提供&#x200B;**「顯示」功能表**。 若要存取[顯示]功能表下可用的各種選項，請瀏覽至[編輯器設定](./config-editor-settings.md)。 「顯示」功能表會繼續在「舊編輯器」的「功能表」下拉式清單中使用。
...
![在區塊元素中插入元素](images/note-in-para-insert-element_cs-editor-2-0.png)
...
在「新編輯器」中，當水準游標出現時，您可以在現有元素之間插入新元素。 此游標表示您的游標位於區塊元素之外，並可在該位置新增元素。

![](images/Block-cursor-editor-2-0.png){width="300"}

例如，在DITA主題中，當水準游標出現在`short description`和`body`之間時，您可以插入`prolog`元素。 然後您可以使用`prolog`新增中繼資料，例如`copyright`、`author`和其他詳細資料。
...
在目前或下一個有效位置插入簡單表格或表格。 選取[表格]圖示以開啟&#x200B;**插入簡單表格**&#x200B;對話方塊。
...
>[!NOTE]
>
> 您也可以選擇切換至新編輯器，以改善表格編輯體驗。 如需新編輯器中可用的表格製作功能的詳細資訊，請檢視[使用新編輯器中的表格](./web-editor-other-features.md#work-with-tables-in-the-new-editor)。
...
**Source路徑**&#x200B;欄位會顯示所選參考的相對路徑。 使用&#x200B;**複製路徑**&#x200B;按鈕來複製絕對路徑。

**Source UUID**&#x200B;欄位會顯示所選參考的UUID。 使用&#x200B;**複製UUID**&#x200B;按鈕複製UUID。
...
在新編輯器中，您也可以透過在製作模式中拖曳影像控點來調整影像大小：

- **轉角控制點**：拖曳轉角控制點會調整影像大小，同時維持其外觀比例。

  ![](images/resize-corner-handles.png)

- **中間控制點**：按下&#x200B;**Shift**&#x200B;以顯示其他中間控制點。 拖曳這些專案可調整影像大小，而不需維持外觀比例，讓您單獨增加或減少高度或寬度。

  ![](images/resize-middle-handles.png)

拖曳時，**內容屬性**&#x200B;面板中的「高度」和「寬度」值會自動更新，以反映新的尺寸。 此調整大小也會反映在&#x200B;**並排式**&#x200B;模式和&#x200B;**預覽**&#x200B;模式中，因此您可以確認調整大小後的影像在發佈前的顯示方式。
...
>[!NOTE]
>
>您也可以在影像中定義可點按區域\（影像地圖\）。 如需詳細資訊，請在[左側面板](web-editor-left-panel.md)區段中檢視&#x200B;**插入/編輯影像地圖**&#x200B;功能說明。
...
>[!IMPORTANT]
>
> 即使參考內容的相對路徑顯示在&#x200B;**連結路徑**&#x200B;屬性中，內部仍會使用參考內容的UUID建立連結。
...
版本資訊會顯示目前版本的主題或地圖。 版本編號旁會出現星號`(*)`，表示變更已儲存在檔案中，但未建立版本。
