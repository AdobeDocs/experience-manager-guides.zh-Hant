...
1. 在&#x200B;**指派給**&#x200B;下拉式欄位中，選取您要將主題傳送給稽核對象的稽核者。 鍵入時，您也可以在選取的專案中依成員名稱或角色名稱搜尋。

   *指派給*&#x200B;欄位只有在您在&#x200B;*專案*&#x200B;欄位中選取使用中的專案之後才會啟用。 系統會根據選取的專案擷取成員或角色清單。 如果未選取任何專案，*指派給*&#x200B;欄位會維持停用狀態。

   您可以將稽核任務指派給專案的個別使用者或使用者群組。 請注意，只有當您是專案的管理員群組的一員時，才能將稽核任務指派給個別使用者，否則您只會在&#x200B;*指派給*&#x200B;欄位中檢視使用者群組。

   >[!NOTE]
   >
   > 稽核工作流程是專案專屬的。 當您建立專案時，您會將專案團隊成員新增至專案，並將他們指派至群組。 因此，當您在這裡選取專案時，您有權選擇屬於該專案的成員。 如需有關專案的詳細資訊，請檢視[建立DITA專案](authoring-create-dita-project.md#)。
...
1. 啟用&#x200B;**自動提醒**，為檢閱者設定排程的提醒通知（和電子郵件提醒）。 您可以在稽核任務的到期日之前和到期之後執行此操作。

   >[!NOTE]
   >
   >在您選取稽核任務的到期日之前，自動提醒切換功能保持停用。

   - **到期日提醒之前**：您最多可以在到期日前60天設定多個提醒（最多3個），以提前通知稽核者。 提醒1可以設定為從今天到到期日的剩餘天數，並且提醒值必須依降序排列，代表到期日期之前的天數。

   - **到期日提醒之後**：您也可以設定在到期日之後傳送的提醒（最多3個），以通知檢閱者工作已逾期。 逾期提醒最多可在到期日後30天內設定。 提醒值必須依遞增順序排列，代表到期日之後的天數。

   系統會根據設定的排程，在計畫的提醒日期傳送AEM通知和電子郵件提醒給稽核者。
...
1. 選取&#x200B;**「下一步」**。
   ...
   >[!NOTE]
   >
   > **下一步**&#x200B;按鈕只有在您提供所有必要欄位的詳細資料之後才會啟用，包括&#x200B;*標題*、*專案*、*指派給*&#x200B;和&#x200B;*到期日*。
