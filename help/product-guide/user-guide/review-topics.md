...
依照預設，如果共用多個主題以供檢閱，則會向檢閱者顯示主題的複合檔案檢視。 若是DITA map檢閱，則地圖中的所有主題會以單一檔案的形式呈現，類似書冊檢視。 如果需要，您也可以選取特定主題，然後複查畫面上只會顯示該主題。
...
您可以透過反白文字、在文字上加亮標示、插入文字或新增註記來新增內嵌註解。 「註解」工具列中提供的不同型別的註解工具說明如下：
...
「註解」面板會顯示目前主題中提供的註解清單。 如果主題傳送給多位稽核者，此面板也會列出其他稽核者的註解。 註解面板中的每個註解都會連結到目前主題中的對應文字。 它有助於您識別註解的文字。 每個註解都會顯示已新增註解及時間戳記的檢閱者姓名。
...
>[!NOTE]
>
> 若要檢視指派給稽核工作的使用者清單並在註解中標籤這些使用者，您必須在`/home/users and /home/groups`節點上具有&#x200B;*讀取*&#x200B;存取權。 如需詳細資訊，請檢視[使用者管理與安全性](../cs-install-guide/user-admin-sec.md#additional-notes-on-user-groups)。<br> 如果在確認存取後仍無法使用標籤，您的管理員可能需要指派`user-admin`授權才能啟用此功能。

![](images/tag-users-review-ui.png){width="350"}

若要標籤使用者，請在註解或回覆中輸入`@`，然後從檢閱者清單中選取使用者。 顯示使用者的顯示名稱及電子郵件地址，以方便識別。 如果電子郵件地址無法使用，則會改為顯示使用者ID。
...
如果您想提供其他檔案中可用的其他資訊來補充您的註解，您可以附加註解來補充。 身為檢閱者，您可以輕鬆將一或多個檔案從本機系統新增至您的註解。 可以將檔案新增至所有支援的註解形式 — 反白顯示、刪除線、插入文字或註解。
...
如果您的主題有條件式內容，您將會檢視右側的&#x200B;**條件** \(![](images/conditions-icon.svg)\)圖示。 選取&#x200B;**條件**&#x200B;圖示會開啟「條件」面板，讓您根據主題中的可用條件反白顯示內容。
...
如果建立稽核任務時附加了一個或多個DITAVAL檔案，則「條件」面板顯示的條件與[「使用條件」面板](#work-with-conditions-panel)區段中所述的條件不同。 每個條件都由切換開關而非眼睛圖示控制，切換已預先設定，以反映附加的DITAVAL檔案中定義的條件，因此您可以依照審閱發起人的預期方式審閱內容。

>[!BEGINTABS]

>[!TAB 條件面板（不含DITAVAL條件）]
...
與只新增或移除條件化內容反白顯示的眼睛圖示不同，在「條件」面板中關閉切換會將該內容隱藏在「檢閱」UI中，因此內容完全不會出現在檔案中。 再次開啟切換開關即可顯示內容。

預覽模式會反映與[使用條件面板](#work-with-conditions-panel)區段中所述的相同切換狀態。

視檢閱發起人設定檢閱工作的方式而定，您可能無法變更這些切換：

- 如果發起人允許變更，您可以開啟或關閉任何切換按鈕，以檢閱不同條件下的內容。

  ![](./images/review-conditions-enabled.png)

- 如果啟動器不允許變更，則會停用切換，且僅反映啟動器的設定。

  ![](./images/review-conditions-disabled.png)

您對切換所做的任何變更只會套用至您目前的作業階段。 重新整理頁面會將切換重設為附加的DITAVAL檔案所定義的狀態。

### 即時檢閱

「註解」面板會以註解以及作者對註解採取的意見反應或動作即時更新。
...
您可以篩選檔案中的註解，以視需要檢視特定註解。 若要篩選註解，請選取出現在「註解」面板中「搜尋註解」文字方塊右邊功能表中的&#x200B;**篩選**&#x200B;圖示\(![](images/filter-search-icon.svg)\)。
