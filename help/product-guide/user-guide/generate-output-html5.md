...
| DITA-OT命令列引數 | 指定在產生輸出時要DITA-OT處理的其他引數。 <br><br>注意：從2502版Experience Manager Guides開始，`generate.copy.outer`引數不再由內部管理。 此變更表示如果您的HTML5內容位於對應目錄之外，您必須在`-Dgenerate.copy.outer=3`DITA-OT命令列引數&#x200B;**欄位中明確設定引數**。 這可確保內容受到正確處理並包含在輸出中。 如需處理對映目錄外部內容的詳細資訊，請檢視[DITA-OT檔案](https://www.dita-ot.org/dev/parameters/generate-copy-outer#:~:text=The%20generate.,located%20outside%20the%20map%20directory/)。 |
