...
>[!NOTE]
>
> 作為管理員，您也可以選擇在編輯器中依檔案名稱來檢視檔案清單。 選取&#x200B;**使用者偏好設定**&#x200B;中&#x200B;**編輯器檔案顯示設定**&#x200B;區段下的&#x200B;**檔案名稱**&#x200B;選項。
...
>[!NOTE]
>
> 從Cloud Service的2025.11.0版和內部部署的5.2.0版開始，**存放庫**&#x200B;已由&#x200B;**檔案總管**&#x200B;取代。 若為5.2版之前的內部部署，您可繼續透過編輯器介面檢視及使用存放庫。
...
![在存放庫視圖中搜尋檔案](images/repository-filter-search.png){width="300"}
...
- **非DITA檔案**：您可以在選取的路徑中搜尋&#x200B;**Ditaval檔案**、**影像檔案**、**多媒體**、**檔案**&#x200B;和&#x200B;**JSON**。
  ...
  ![快速搜尋篩選器](images/repository-filter-search-quick.png) {width="300"}
...
![進階篩選器對話方塊](images/repository-filter-search-advanced.png) {width="650"}
...
- **上傳資產**：將檔案從您的本機系統上傳至Adobe Experience Manager存放庫中選取的資料夾。 您也可以將檔案從本機系統拖放至目前的工作主題上。 如果您想要將本機系統中的影像插入主題中，這會非常有用。
  ...
  <summary>處理檔案名稱中的無效字元</summary>

  如果上傳資產的檔案名稱包含無效字元(例如* / ： [\] | # % { } ？)，可能會出現以下情況：

  - **部分上傳**：表示上傳的一個或多個資產的檔案名稱中包含無效字元。

    ![](images/partial-asset-upload.png)

  - **上傳失敗** ：表示所有要上傳的資產在其檔案名稱中包含無效字元。

    ![](images/upload-asset-failed.png)

  若要解決這些衝突，請從資產的檔案名稱中移除任何無效字元，然後重新上傳至存放庫。
...
存放庫檢視中檔案的![選項功能表](images/options-menu-repo-view-file-level.png){width="550"}
...
- **屬性**：使用此開啟所選檔案的屬性頁面。
  ...
  任何新增、刪除或修改此頁面的中繼資料屬性（無論是預設或自訂）都將觸發檔案版本上的[工作復本指標](./web-editor-edit-topics.md#working-copy-indicator)。

  您也可以從Assets UI中存取「屬性」頁面，方法是選取檔案，然後選取工具列中的「屬性」圖示。
...
Explorer提供存放庫先前提供的大部分功能，包括：

- 瀏覽檔案和資料夾
- 檔案和資料夾的[選項]功能表

但是，它引入了增強的搜尋和篩選體驗，旨在提高可用性和效率。

如需檔案與資料夾內容功能表的詳細資訊，請檢視[選項功能表](#options-menu)。

如需有關搜尋體驗的詳細資訊，請檢視[搜尋面板](./search-panel-explorer.md)。
...
地圖檢視中的![索引鍵](images/view-key-title-map-view.png){width="300"}
...
- **在檔案總管**&#x200B;中尋找：顯示對應檔案在檔案總管\（或DAM\）中的位置。
...
- **執行健康狀態檢查**：啟用此選項以在選取的對應上執行健康狀態檢查。 如需有關此功能的詳細資訊，請在地圖編輯器中檢視[其他功能](./map-editor-other-features.md#run-health-check-on-a-map)。
  >[!NOTE]
  >
...
- 選取[可重複使用的內容]旁的&#x200B;**+**&#x200B;圖示以開啟&#x200B;**選取檔案**&#x200B;對話方塊。
- 選取您要新增的檔案，然後選擇&#x200B;**選取**。 您也可以使用篩選搜尋選項來搜尋特定檔案。 如需詳細資訊，請在編輯器中檢視[其他功能](./web-editor-other-features.md)。
...
如果您的管理員已建立屬性的設定檔，則您將會取得這些屬性及其設定的值。 您也可以在&#x200B;**Workspace設定**&#x200B;的&#x200B;**顯示屬性**&#x200B;標籤下指派系統管理員設定的顯示屬性（在5.2 **之前的**&#x200B;內部部署設定中顯示為&#x200B;**設定**）。 為元素定義的屬性會顯示在「配置圖」和「大綱」檢視中。
...
您也可以將條件限製為主題中的某些元素。使用`<elementdef>`標籤來定義元素，並使用`<attributedef>`標籤來定義可套用至元素的條件。 如果您未新增`<elementdef>`標籤，可以將條件套用至所有元素。
...
若要從「條件」面板檢視主旨配置，您的系統管理員必須在&#x200B;**Workspace設定** （針對5.2 **之前的**&#x200B;內部部署設定，顯示為&#x200B;**設定**）的「一般」標籤下，選取&#x200B;**在「條件」面板中顯示主旨配置**&#x200B;選項。 如需詳細資訊，請檢視[索引標籤列](./web-editor-tab-bar.md)。
...
1. 選取「程式碼片段」旁的&#x200B;**+**&#x200B;圖示，以開啟&#x200B;**新增程式碼片段**&#x200B;對話方塊。

   ![](images/snippet-new-dialog.png){width="300"}

   在「新增程式碼片段」對話方塊中，提供出現在「程式碼片段」面板中的「標題」、「說明」，選取內容的格式（DITA或HTML），並提供您要建立的程式碼片段內容的程式碼。 選取&#x200B;**建立**&#x200B;以儲存並建立程式碼片段。

2. 在內容編輯區域中，以滑鼠右鍵按一下要當作程式碼片段使用的元素階層連結，然後從內容功能表中選擇&#x200B;**建立程式碼片段**。 「新程式碼片段」對話方塊會出現，並在&#x200B;**Content**&#x200B;欄位中填入所選專案的XML程式碼。 輸入程式碼片段的&#x200B;**標題**&#x200B;和&#x200B;**描述**，並選取&#x200B;**建立**&#x200B;以儲存程式碼片段。

3. 在內容編輯區域中，以滑鼠右鍵按一下要當作程式碼片段使用的內容上任何位置，然後從內容功能表中選擇&#x200B;**建立程式碼片段**。 「新程式碼片段」對話方塊出現，並在&#x200B;**Content**&#x200B;欄位中填入所選專案的XML程式碼。 輸入程式碼片段的&#x200B;**標題**&#x200B;和&#x200B;**描述**，並選取&#x200B;**建立**&#x200B;以儲存程式碼片段。
...
- 從「程式碼片段」面板中選取程式碼片段，並將其拖放至主題中所需的位置。 您也可以使用「代碼片段」面板頂端的篩選選項，來調整您的檢視：

  - **顯示所有程式碼片段**：列出所有可用的程式碼片段，包括DITA和HTML格式。
  - **僅顯示適用的片段**：篩選清單以僅顯示與您目前主題或內容相關的片段。 例如，如果您處理DITA主題，HTML片段將從清單中排除，以確保內容準確性。
...
「尋找和取代」功能可用於兩種不同的模式，具體取決於您的需求：

- **使用Source模式：**&#x200B;搜尋不僅會掃描可見的內容，還會掃描搜尋字串的基本來源內容（XML結構，包括元素、標籤和屬性值）。 此模式可確保在整個內容中進行全面搜尋。 若要使用此功能，您必須啟用[尋找和取代]面板中可用的&#x200B;**使用來源模式**&#x200B;選項。

  >[!NOTE]
  >
  > **使用來源模式**&#x200B;功能適用於2026.01.0版的Experience Manager Guides as a Cloud Service以及5.2版的內部部署設定。 若要使用此功能，必須先根據您使用的設定為內容編制索引。
  > - **對於雲端服務**：必須執行自訂索引部署。 如需詳細資訊，請檢視[部署自訂索引](/help/product-guide/cs-install-guide/custom-indexing.md)。 完成後，請聯絡您的客戶成功團隊以啟用此功能。
  > - **對於內部部署**：需要重新索引現有內容，才能使用功能。 如需詳細資訊，請檢視[重新索引內容](/help/product-guide/install-guide/custom-indexing-prem.md)。 完成後，請聯絡您的客戶成功團隊以啟用此功能。
...
<details>
    <summary> 執行全域搜尋並以Source模式取代</summary>

若要執行全域搜尋並以Source模式取代，請執行以下步驟：

1. 開啟全域&#x200B;**尋找及取代**&#x200B;面板。
1. 啟用&#x200B;**使用來源模式**&#x200B;選項。
1. 選取&#x200B;**路徑**&#x200B;下拉式清單，並選取下列其中一個選項來執行搜尋。

   - **路徑**：若要搜尋選取的路徑
   - **對應**：若要從存放庫或集合中搜尋某些特定對應
   - **目前對應**：若要搜尋目前開啟的對應

   ![](images/path-dropdown-source-mode.png){width="350"}


1. 在&#x200B;**尋找**&#x200B;欄位中輸入搜尋字串。 若要縮小結果範圍，請選取[尋找]欄位旁的&#x200B;**篩選器**&#x200B;圖示，並選取下列需要的篩選器：

   ![](images/find-filters-source-mode.png){width="350"}

   - **檔案型別**：選擇檔案型別；**主題**，以及要搜尋文字的&#x200B;**對應**
   - **檔案狀態**：您可以從可用選項中選取檔案狀態。 顯示的檔案狀態選項衍生自「資料夾」設定檔。 它們代表這些設定檔中所有可能檔案狀態的組合集。 預設狀態包括「稽核中」、「完成」、「草稿」、「已核准」、「編輯」和「已檢閱」。
   ...
   - **其他**：您可以設定下列專案：
     - **標籤**：根據標籤篩選內容。
     - **區分大小寫的搜尋**：啟用搜尋功能，確保結果符合指定的確切字母大小寫。
     - **列出其他使用者鎖定的檔案**：顯示其他使用者目前鎖定的檔案，在解除鎖定前無法編輯。

1. 按Enter或選取&#x200B;**搜尋**&#x200B;圖示以執行搜尋。

   ![](images/search-icon.png){width="350"}

1. 從搜尋結果清單中選取檔案。 檔案會在Source檢視中開啟，搜尋字詞會在內容中反白顯示。

1. 在&#x200B;**取代為**&#x200B;欄位中輸入您要用來取代的字詞。 若要自訂取代的套用方式，請按一下欄位旁的&#x200B;**設定**&#x200B;圖示，然後從可用的選項中選擇。

   - **取代解除鎖定的檔案**：選取此選項可允許取代解除鎖定的檔案。

   - **取代後建立新版本**：如果您要建立新版本的主題，並選擇取代文字，請選取此選項。 您也可以提供版本註解，這些註解將與每個更新的檔案一起新增。 如果未選取此選項，則變更會儲存在主題的目前版本中，不會建立新版本。

   ![](images/replace-settings-source-mode.png){width="350"}


1. 選取&#x200B;**取代出現位置**&#x200B;以取代主題中目前反白的搜尋字串，或選取頂部和底部箭頭以移動到文字的下一個或上一個出現位置。
...
1. 選取「全部取代」**&#x200B;**，以指定的取代字串取代所有搜尋檔案中搜尋字串的所有專案。 取代所有專案後，您將會看到通知。
   ...
   >[!NOTE]
   >
   >若要啟用「**全部取代**」按鈕，您的資料夾設定檔管理員或系統管理員必須在&#x200B;**Workspace設定** （針對5.2 **之前的**&#x200B;內部部署設定，顯示為&#x200B;**設定**）的&#x200B;**一般**&#x200B;索引標籤下，選取選項&#x200B;**啟用全部取代**。

1. 您也可以將滑鼠移到搜尋結果清單的檔案上，以檢視其右邊的&#x200B;**全部取代檔案**&#x200B;圖示，讓您取代單一檔案中所有出現的字詞。

   >[!NOTE]
   >
   > 您也會取得&#x200B;**移除**&#x200B;圖示，以從搜尋結果中移除檔案。 您移除的檔案會移出清單，而且搜尋字詞不會在其中被取代。

   ![](images/replace-all-in-file.png){width="350"}

1. 「全部取代」作業完成後，會產生可下載的CSV報表，提供所執行全部取代動作的快照。 您可以下載報告以檢視有關取代操作的詳細資訊，包括成功取代的相符專案數、每個相符專案的詳細資訊，以及任何失敗及其相應原因。 作業可能會因為某些特定原因而失敗，例如檔案被其他使用者鎖定、作業期間對檔案進行的變更導致的驗證錯誤或其他類似問題。

   ![](images/snapshot-replace-all.png){width="350"}

在整個系統中，一次只能執行一個取代所有操作，並且在執行操作之前，您將會檢視「取代所有進行中」狀態。 您也可以中止介於兩者之間的取代所有操作。 如果中止操作，您會在收件匣中收到有關該操作的通知。
...
- **不使用Source模式：**&#x200B;搜尋僅限於在[作者]檢視中顯示的文字，僅著重於略過XML元素或屬性等來源內容的可見內容。 此模式適用於快速、僅限內容的搜尋。

  ![](images/map-find-replace-without-source-mode.png)

<br>
<details>
    <summary> 不使用Source模式執行全域搜尋和取代</summary>


若要在沒有Source模式的情況下執行全域搜尋和取代，請執行下列步驟：

1. 開啟全域&#x200B;**尋找及取代**&#x200B;面板。
1. 選取&#x200B;**路徑**&#x200B;下拉式清單，然後選擇下列其中一個選項來執行搜尋。
...
1. 從搜尋結果清單中選取檔案。 檔案會在內容編輯區域的「作者」檢視中開啟，搜尋字詞會在內容中反白顯示。

1. 在&#x200B;**取代為**&#x200B;欄位中輸入您要用來取代的字詞。 若要自訂取代的套用方式，請按一下欄位旁的&#x200B;**設定**&#x200B;圖示，然後從可用的選項中選擇。

   - **取代前鎖定檔案**：如果您要在取代搜尋字串前自動鎖定檔案，請選取此選項。 如果您的管理員已啟用設定，以在編輯前鎖定檔案，此設定會更相關。 啟用後端設定後，您應選取此選項。 這會防止檔案鎖定對話方塊提示您在進行任何變更之前鎖定每個檔案。 如果您未選取此選項，則在開啟檔案進行編輯之前，將會出現提示。
...
1. 選取&#x200B;**取代出現位置**&#x200B;以取代主題中目前反白的搜尋字串，或選取頂端和底部箭頭以移動到文字的下一個或上一個出現位置

   ![](images/replace-occurrence.png){width="350"}

1. 選取&#x200B;**全部取代**，以單一步驟中的取代字詞取代單一檔案中搜尋字詞的所有出現位置。 取代所選檔案中的所有相符項後，您將會看到通知。

   >[!NOTE]
   >
   > 若要啟用「然後&#x200B;**全部取代**」圖示，您的資料夾設定檔管理員或系統管理員必須在&#x200B;**Workspace設定** （針對5.2 **之前的**&#x200B;內部部署設定，顯示為&#x200B;**設定**）中的&#x200B;**一般**&#x200B;索引標籤下，選取&#x200B;**啟用全部取代**&#x200B;選項。 如果在取代操作期間發生錯誤，則會因為XML剖析問題或DITA相關錯誤而略過這些檔案。

   ![](images/replace-all.png){width="350"}

1. 您也可以將滑鼠停留在搜尋結果清單的檔案上，以檢視其右邊的&#x200B;**全部取代檔案**&#x200B;圖示，讓您取代單一檔案中所有出現的字詞。
