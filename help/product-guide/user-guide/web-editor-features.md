...
搜尋是即時的，這表示當您在 **尋找** 欄位，系統會立即搜尋並在主題中選取字詞。 同樣地，若要取代主題中的文字，請在個別欄位中輸入搜尋字詞及其取代字詞，然後按一下 **取代** 或 **全部取代** 按鈕。
...
- **一般**：一般設定可讓您設定要與網頁編輯器搭配使用的字典。 此標籤包含三個區段： **拼字檢查**， **條件**、和 **製作**.
  ...
  ![](images/editor-setting-general.png){width="650" align="left"}
   ...
   - **拼字檢查**：有兩個選項 —  **AEM拼字檢查** 和 **瀏覽器拼字檢查**. 依預設，編輯器會使用瀏覽器拼字檢查功能，其中拼字檢查是使用瀏覽器的內建字典執行。 您可以切換至AEM拼字檢查以使用AEM字典，也可以自訂以新增自訂單字清單。 如需自訂AEM字典的詳細資訊，請參閱 *自訂AEM預設字典* 安裝與設定Adobe Experience Manager Guidesas a Cloud Service中的區段。
...
- **翻譯**：此標籤包含建立語言群組、將來源標籤傳播至目標版本，以及清除翻譯專案的選項。
  ![](images/editor-setting-translation.png){width="550" align="left"}

   - **語言群組**：身為管理員，您可以建立語言群組，並將它們當做一組使用，以翻譯內容。\
     執行以下步驟來建立新的語言群組：
      1. 選取新增 ![新增圖示](images/Add_icon.svg) 圖示。
     ...
     輸入語言的前幾個字元，或輸入語言代碼以篩選所需的語言。 例如，輸入&#39;en&#39;以篩選名稱或程式碼開頭包含&#39;en&#39;的所有語言。
      1. 選取 **完成** 將選取的語言新增至群組。 隨即顯示語言。 新增三種或多種語言時， **顯示更多** 顯示。 您可以選取 **顯示更多** 以檢視群組中的所有語言。
         >[!TIP]
         >
         > 切換 **顯示更多** 至 **顯示較少** 並且只檢視幾種語言。

      1. 將滑鼠懸停在群組中要編輯的語言上 ![編輯圖示](images/edit_pencil_icon.svg) 或刪除 ![刪除](images/Delete_icon.svg) 語言群組。
      1. 儲存 **編輯器設定**.
   ...
   - **翻譯專案完成後的清理**：選取此選項可設定在翻譯後自動停用或刪除的翻譯專案。 根據預設， **無** 已選取，因此專案在翻譯後就會存在。

     如果您想稍後再使用翻譯專案，可以停用這些專案。 刪除專案會永久刪除專案中存在的所有檔案和資料夾。


- **中繼資料**：您可以控制主題的版本中繼資料及其值顯示在 **版本記錄** 對話方塊。  在中繼資料路徑中，指定您要從中挑選中繼資料的節點位置。 您也可以為中繼資料定義自訂名稱作為標籤。 預設屬性為「標題」、「檔案狀態」和「標籤」。

  中繼資料可從下的任何屬性中挑選 `/jcr:content` 節點，因此您可以將屬性的路徑新增為中繼資料路徑。
  ...
  ![編輯器設定中的「中繼資料」標籤](images/editor-setting-metadata.png){width="550" align="left"}
...
- **一般**：一般標籤可讓您進行下列設定：
  ...
  ![使用者偏好設定的「一般」標籤](images/user_preference_editor.PNG){width="550" align="left"}

   - **資料夾設定檔**：資料夾設定檔控制與條件屬性、製作範本、輸出預設集和網頁編輯器設定相關的各種設定。 預設會顯示「全域設定檔」。 此外，如果您的管理員在系統中設定了資料夾設定檔，則這些資料夾設定檔也會顯示在「資料夾設定檔」清單中。

     管理員可以在資料夾設定檔中定義的網頁編輯器設定包括：自訂使用者介面，包括工具列圖示、網頁編輯器的版面、片段和根對應。 如需詳細資訊，請參閱 *設定全域或資料夾層級設定檔* 在「安裝與設定Adobe Experience Manager Guides」as a Cloud Service中。
...
- **外觀**：選取網頁編輯器應用程式的主題，以及內容編輯區域的來源檢視。

  ![使用者偏好設定的外觀標籤](images/user_preference_editor_appearance.png){width="550" align="left"}
   ...
   - **應用程式主題**：您可以從 **淺色** 或 **深色** 應用程式的主題。 若為 **淺色** 主題，工具列和面板會使用淺灰色的背景。 若為 **深色** 主題，工具列和面板會使用黑色背景。 選取 **使用裝置主題** 允許「Experience Manager指南」根據裝置主題選取淺色和深色主題。  在所有主題中，內容編輯區域會在 **作者** 檢視。

   - **來源檢視主題**： — 您可從 **淺色** 或 **深色** 來源檢視中內容編輯區域的主題。 若為 **淺色** 佈景主題，內容編輯區域會使用淺灰色的背景作為來源檢視，若為 **深色** 佈景主題，使用黑色背景。 選取 **使用裝置主題** 允許「Experience Manager指南」根據裝置主題選取淺色和深色主題。

   - **一律在存放庫中尋找檔案**：選取此選項，以在網頁編輯器中編輯檔案時，顯示檔案在存放庫中的位置。

   - **在製作模式中顯示不斷行空格指示器**：選取此選項，以在網頁編輯器中編輯時顯示不間斷空格的指示器。 預設為啟用。
...
在目前或下一個有效位置插入有效元素。 您也可以使用鍵盤快速鍵 ***Alt***+***輸入*** 以開啟「插入元素」快顯功能表。 例如，您正在編輯一個段落，然後在 **插入元素** 快顯視窗中會顯示可插入段落中的元素清單。 選取您要插入的元素。 您可以使用鍵盤捲動元素清單並按 ***輸入*** 以插入必要的元素。

您可以檢視兩種有效元素：

- **目前位置的有效元素**：清單會顯示您可在目前游標位置本身插入的元素。

- **目前位置以外的有效元素**：清單會顯示您可在元素階層內目前元素的任何父項之後插入的元素。



例如，如果您位於內嵌 `<b>` 元素，您可以插入如下的元素 `<u>`， `<xref>`， `<i>` 在目前位置。 相反地，您可以插入類似以下的元素 `<table>` 和 `<topic>` 位於目前位置之外。
...
*輸入&#39;t&#39;以搜尋所有以&#39;t&#39;開頭的有效元素。*

如果您在區塊元素內工作，例如 `note`，然後使用插入元素圖示，在 `note` 元素。 在下列熒幕擷圖中，筆記元素已插入p \(paragraph\)元素內：
...
輸入新元素的另一種方法是使用快顯選單。 在檔案中的任何位置按一下滑鼠右鍵，以叫用前後關聯功能表。 從此功能表選擇 **插入元素** 以顯示 **插入元素** 對話方塊，並選擇要插入的元素。
...
>[!NOTE]
>
>您也可以選取 **分割清單** 從清單專案的前後關聯功能表中選取選項，以分割目前清單並開始相同層級的新清單。
...
此 **版本記錄** Web編輯器中的功能可讓您檢查DITA檔案的可用版本、比較這些版本，以及從編輯器本身還原為任何版本。
...
1. 選擇您要比較或要在中還原的主題版本 **比較對象：** 下拉式清單。
...
1. 啟用 **檢視標籤和註解** 選項可檢視套用至目前版本和比較版本的標籤和註解。

1. 您也可以檢視下列資訊： **版本記錄** 對話方塊：

   **預覽** tab：新加入的內容是綠色字型，而刪除的內容是紅色字型。

   **中繼資料** tab：新加入的中繼資料為綠色字型，而刪除的中繼資料為紅色字型。
...
1. 從下拉式清單中選擇版本後， **還原至選取的版本選項** 變為可用。 預覽視窗會顯示目前版本與所選主題版本之間的差異。
...
![在存放庫檢視中搜尋檔案](images/repository-filter-search.png){width="300" align="left"}

*套用篩選器以搜尋包含文字的檔案`general purpose.`*

選取 **篩選搜尋** \(![搜尋篩選器圖示](images/filter-search-icon.svg)\)圖示開啟「篩選依據」快顯視窗。

>[!NOTE]
>
> 當您搜尋任何文字或篩選任何檔案時， **篩選搜尋**  \(![搜尋篩選器圖示](images/filter-search-icon.svg)\)圖示表示我們位在「搜尋」面板上，且已套用部分篩選器。


您有以下選項可篩選檔案，並在Adobe Experience Manager存放庫中縮小搜尋範圍：

- **DITA檔案**：您可全部搜尋 **DITA主題** 和 **DITA對應** 出現在選取的路徑上。 預設會選取這些專案。
- **非DITA檔案**：您可以搜尋 **Ditaval檔案**，  **影像檔案**， **多媒體**， **檔案**、和 **Json** 在選取的路徑中。
...
選取 **進階篩選** ![進階篩選器圖示](images/advanced-filter-gear-icon.svg)圖示以檢視 **進階篩選** 對話方塊。

您可以在「 」下檢視下列選項 **一般** 和 **進階** 索引標籤。
...
這與存放庫視窗中的搜尋方塊同步。 例如，如果您輸入 `general purpose` 在「存放庫」面板上的搜尋方塊中，它也會出現在 **進階篩選** 對話方塊，反之亦然。

- **搜尋**：選取您要搜尋Adobe Experience Manager存放庫中檔案的路徑。

- **簽出者**：您可以尋找指定使用者簽出的檔案。
- **上次修改時間**：您可以尋找在選定日期之後但在選定日期之前上次修改的檔案。
- **修改日期早於**：您可以尋找在選定日期之前上次修改的檔案。
- **時間段**：您也可以尋找過去兩小時、上週、上個月或去年上次修改的檔案。
...
- 選取 **關閉篩選器** ![關閉圖示](images/close-icon.svg) 圖示關閉篩選器並返回存放庫的樹狀檢視。
  >[!NOTE]
  >
  >您的系統管理員也可以設定文字篩選器，並顯示或隱藏其他篩選器。 如需詳細資訊，請參閱 *設定文字篩選* 安裝與設定Adobe Experience Manager Guidesas a Cloud Service中的區段。

  會顯示包含搜尋文字的已篩選檔案清單。 例如，包含文字的 `general purpose` 列於前一個熒幕擷圖中。 您可以從篩選的清單中選取多個檔案，將其拖放到開啟以進行編輯的對映中。
...
- **上傳資產**：從您的本機系統上傳檔案至Adobe Experience Manager存放庫中選取的資料夾。 您也可以將檔案從本機系統拖放至目前的工作主題上。 如果您想要將本機系統中的影像插入主題中，這會非常有用。
...
![存放庫檢視中檔案的選項功能表](images/options-menu-repo-view-file-level.png){width="550" align="left"}
...
在 **下載地圖** 對話方塊中，您可以選擇下列選項：
  ...
  在您按一下 **下載** 按鈕中，對應匯出套件請求已排入佇列。 此 **成功** 如果成功建立封裝，便會顯示對話方塊。  您可以按一下 **下載** 按鈕來自 **成功** 對話方塊。
...
下列範例說明如何在AEM Guides中使用主旨配置。

1. 在您選擇的工具中建立主旨配置檔案。 下列XML程式碼會建立主旨配置，以繫結 `platform` 屬性。
...
1. 將父對映設定為根對映 **使用者偏好設定**. 一旦將此對映新增為根對映後，主旨配置便會顯示在「主旨配置」面板中。
...
- 按一下「代碼片段」旁的+圖示，開啟「新增代碼片段」對話方塊。
  ...
  在「新增程式碼片段」對話方塊中，提供顯示在「程式碼片段」面板中的標題、說明，以及您要建立的程式碼片段內容的XML程式碼。 按一下 **建立** 儲存並建立程式碼片段。

- 在內容編輯區域中，以滑鼠右鍵按一下要當作程式碼片段使用的元素階層連結，然後選擇 **建立代碼片段** 從內容功能表。 「新程式碼片段」對話方塊隨即顯示，並會將所選元素的XML程式碼填入 **內容** 欄位。 輸入 **標題** 和 **說明** 取得程式碼片段，然後按一下 **建立** 儲存程式碼片段。

- 在內容編輯區域中，以滑鼠右鍵按一下要當作程式碼片段使用的內容上任何位置，然後選擇 **建立代碼片段** 從內容功能表。 「新程式碼片段」對話方塊隨即顯示，並會將所選元素的XML程式碼填入 **內容** 欄位。 輸入 **標題** 和 **說明** 取得程式碼片段，然後按一下 **建立** 儲存程式碼片段。
...
「範本」面板僅供管理員使用。 使用此面板，管理員就可輕鬆建立和管理範本，然後供作者使用。 依預設，範本分類在 *地圖* 和 *主題* 輸入範本。
...
>[!NOTE]
>
> 身為管理員，您也可以選擇在Web編輯器中檢視檔案清單。 選取 **檔案名稱** 的選項 **檢視檔案依據** 中的區段 **使用者偏好設定** ![](images/user_preference_editor_icon.svg).

若要建立範本，請按一下「範本」旁的+圖示，然後選擇要建立的範本。 如果您選取 **主題範本**，則會顯示建立新主題範本對話方塊：
...
使用現有範本上的「選項」功能表，您可以選擇 **編輯** 或 **複製** it. 如果重複，則會保留範本的結構和型別\（屬於document\），您可以重複使用它來建立另一個範本。

**檢閱** -  ![](images/active-review-tasklist-icon.svg)

AEM Guides提供可在您的專案中顯示所有稽核任務的功能。 您可以在檢閱專案中檢視您所屬的所有檢閱專案和作用中檢閱任務。 **檢閱** 面板。  然後，您可以開啟稽核任務以檢視各種稽核者的註釋。
...
1. 選取檢閱 ![](images/active-review-tasklist-icon.svg)   位於左側面板。 此 **檢閱** 面板隨即開啟。  系統會顯示您所屬之審閱專案中的所有審閱專案與作用中審閱任務。
...
1. 您也可以以下列方式篩選專案：
   ...
   - 選取 ![](images/filter-search-icon.svg)  以開啟 **篩選** 對話方塊。 您可以選取所有專案或僅選取特定專案。 所選的專案會列在 **檢閱** 面板。
     ![](images/active-review-select-project.png){width="300" align="left"}

     此 **由我初始的任務** 選項預設為啟用。 它可讓您僅檢視您已起始的任務。

1. 依預設，在您的稽核專案中，您將檢視含有與其相關之註解的主題平面清單。 從左側邊欄套用所需的篩選器，以根據主題中存在的稽核評論來篩選主題：

   - **檢視所有主題**：列出專案中存在的所有主題。
   - **檢視含有註解的主題**：僅列出包含稽核評論的主題。
1. 您也可以輸入要在主題標題或檔案路徑中尋找的搜尋字詞或文字。 會列出標題或檔案路徑中包含辭彙的主題。
1. 連按兩下任何主題，以在作者檢視中開啟。 您可以在下列位置檢視註解： **註解** 面板。
   ![](images/active-review-task-comments.png){width="800" align="left"}


   >[!NOTE]
   > 
   > 此 **檢閱** 面板和 **註解** 面板會一直保持同步。 在「註解」面板中，註解會根據「審閱」面板中載入的稽核任務載入。
   > 如需如何處理註解的詳細資訊，請檢視 [處理稽核意見](review-address-review-comments.md#).
...
>[!NOTE]
>
> 身為管理員，您也可以選擇在Web編輯器中檢視檔案名稱清單。 選取 **檔案名稱** 的選項 **檢視檔案依據** 中的區段 **使用者偏好設定** ![](images/user_preference_editor_icon.svg).
...
1. 您可以按一下 **選項** 下拉式清單，然後從下列選項中選擇：
   ...
   - **取代前簽出檔案**：如果您想要在取代搜尋字詞之前自動簽出檔案，請選取此選項。 如果您的管理員已啟用設定，以在編輯前簽出檔案，此設定會更相關。 啟用後端設定後，您應選取此選項。 這會防止檔案出庫對話方塊在進行任何變更之前提示您出庫每個檔案。 如果您未選取此選項，則在開啟檔案進行編輯之前，將會出現提示。
   - **僅限全字**：如果您想要搜尋整個搜尋字串，請選取此選項。 例如，如果您在搜尋字串中輸入over，則搜尋結果會傳回所有包含單字（如over和overview）的檔案。 如果您要限制搜尋以傳回輸入的確切字詞，請選取此選項。
   - **取代後建立新版本**：如果您要建立新版本的主題，並選取要取代文字的主題，請選取此選項。 您也可以提供版本註解，這些註解將與每個更新的檔案一起新增。

     如果未選取此選項，則變更會儲存在主題的目前版本中，不會建立新版本。

   - **包含間接參考**：如果您想要搜尋間接參照（也在DITA map中）中的字串，請選取此選項。 依預設，這會停用，所以僅對直接參照執行搜尋。

1. 輸入您要尋找的搜尋字詞或文字。
1. 輸入要取代搜尋字詞的文字。
1. 按下Enter或選取 **搜尋** 圖示\( ![](images/search-icon.svg)\)以執行搜尋。
...
1. 按一下 **取代單一專案** \( ![](images/replace-icon.svg)\)以取代主題中目前反白的搜尋字詞，或按一下「下一個相符」 ![](images/next-match-in-search.png) 或 ![](images/previous-match-in-search.png) 上一個相符項，移至文字的下一個或上一個相符項。

1. 按一下 **取代檔案中的所有專案** \( ![](images/replace-all-in-file-icon.svg)\)按一下即可使用取代字詞取代單一檔案中搜尋字詞的所有具體值。 取代所選檔案中的所有相符專案後，您會看到通知。

   >[!NOTE]
   >
   > 從搜尋結果清單將滑鼠懸停在檔案上，在檔案右側會看到「以檔案取代全部」圖示。 您也可以取得「忽略檔案」圖示，從搜尋結果中移除檔案。 您略過的檔案會從清單中移除，而且搜尋字詞不會在其中被取代。

1. 按一下 **全部取代** \( ![](images/replace-all-in-file-icon.svg)\)按一下滑鼠右上角的「 」，即可使用取代字詞取代所有檔案中搜尋字詞的所有具體值。
...
您也可以使用 **在地圖中尋找** 選項來自 **選項** 地圖功能表以尋找和取代地圖中的文字。 此選項會顯示在存放庫面板或地圖檢視中開啟的地圖上。
...
>[!NOTE]
>
> 身為管理員，您也可以選擇在索引標籤中依檔案名稱檢視檔案清單。 選取 **檔案名稱** 的選項 **檢視檔案依據** 中的區段 **使用者偏好設定** ![](images/user_preference_editor_icon.svg).
...
您可以存取 **內容屬性** 功能，方法是選取 **內容屬性** 圖示加以搜尋。 此 **內容屬性** 面板包含檔案中目前所選元素的型別及其屬性的相關資訊。

**型別**：您可以在下拉式清單中檢視並選取目前標籤的完整階層標籤。

**屬性**：此 **屬性** 「配置」、「作者」和「來源」檢視中都有下拉式面板。 您可以輕鬆地新增、編輯或刪除屬性。

1. 按一下 **+新增**.

   ![內容屬性中的屬性](images/properties-tab-attributes_cs.png){width="300" align="left"}

1. 在 **屬性** 下拉式面板，從下拉式清單中選取屬性並指定屬性的值。  然後按一下 **新增**.

   ![具有多個屬性的屬性面板 ](images/attributes-multiple-properties.png){width="300" align="left"}

1. 若要編輯屬性，請將滑鼠指標暫留在該屬性上並選取 **編輯** ![編輯圖示](images/edit_pencil_icon.svg).
   ![編輯屬性](images/edit-attributes-content-properties.png){width="300" align="left"}

1. 若要刪除屬性，請將游標停留在它上並選取 **刪除** ![delete-icon](images/Delete_icon.svg).
...
按一下「檔案屬性」，檢視所選檔案的屬性 ![](images/topic-properties-icon.svg) 圖示加以搜尋。 「檔案屬性」功能可用於所有四種模式或檢視：「版面」、「作者」、「來源」和「預覽」。
...
**注意：** 所有「使用於」和「輸出」參照都會超連結至檔案。 您可以輕鬆開啟及編輯連結的檔案。

除了開啟檔案外，您還可以使用下列工具執行許多動作： **選項** 「參照」區段中的選單。 您可以執行的某些動作包括「編輯」、「預覽」、「複製UUID」、「複製路徑」、「加入我的最愛」、「屬性」和「開啟對映」儀表板。
...
如需詳細資訊，請參閱 [處理稽核意見](review-address-review-comments.md#).
