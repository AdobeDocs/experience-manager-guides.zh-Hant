...
1. 選取「**新增**」。
   ...
   ![新](images/new-aem-sites-dialog-box.png){width="300"}
...
![新](images/aem-sites-new-config.png){width="650"}
...
| 清理先前產生的頁面 | - **刪除從對映中移除之主題先前產生的頁面**：如果DTIA對映的結構變更，您可以使用此選項來移除先前產生之主題頁面。 此功能僅適用於完整地圖發佈。<br><br>假設您已發佈DITA map，其中包含主題a.dita、b.dita和c.dita。 再次發佈對應之前，您已從對應移除b.dita主題。 現在，如果您已選取此選項，則會從AEM Sites輸出中移除與b.dita相關的所有內容，並且僅發佈a.dita和c.dita。<br><br>**注意**：有關已刪除頁面的資訊也會擷取到輸出產生記錄中。 如需有關存取記錄檔的詳細資訊，請[檢視並檢查記錄檔](generate-output-basic-troubleshooting.md#id1821I0Y0G0A__id1822G0P0CHS)。 <br><br>**警告**：刪除主題時，已發佈網站的頁面會變成無法使用。 因此，在刪除主題之前，會顯示警告。 您必須確認刪除它們。<br><br>- **刪除此路徑上其他來源建立的所有頁面**：如果您選取此選項，會刪除其他地圖、個別主題或任何其他來源發佈在此路徑上的所有頁面。 已發佈網站的頁面也會變得無法使用。 因此，在刪除主題之前，會顯示警告。 您必須確認以刪除它們。 |
| 貼文產生工作流程 | 選擇此選項時，會顯示新的「產生後工作流程」下拉式清單，其中包含AEM中設定的所有工作流程。 您必須選取要在輸出產生工作流程完成後執行的工作流程。 |

>[!NOTE]
>
>如果您的資料夾設定檔已設定[健康狀態檢查功能](../install-conf-guide/conf-health-check-preset.md)，則[一般]索引標籤中會顯示額外的&#x200B;**在產生輸出前執行健康狀態檢查**&#x200B;切換。 使用此預設集，您每次使用此預設集產生輸出時，就會自動執行健康情況檢查，因此您不必從對應手動觸發。 報表會附加至發佈記錄檔，且僅供參考。 即使檢查發現未解決的錯誤或警告，它也不會封鎖或延遲您的輸出。 深入瞭解[在Experience Manager Guides](./map-editor-other-features.md#run-health-check-on-a-map)中使用健康狀態功能檢查。
...
![aem網站主題清單](images/aem-presets-topic-list.png)
...
![新](images/aem-sites-preset-legacy-config.png){width="500"}
...
| AEM Sites選項 | 說明 |
| --- | --- |
| 網站名稱 | 輸出儲存在AEM存放庫中的網站名稱。<br><br>AEM存放庫中的節點會以此處指定的名稱建立。 如果您未指定「場地名稱」，則會以DITA map檔案名稱建立場地節點。<br><br>您在此指定的網站名稱也會當作瀏覽器標籤中的標題。<br><br>您也可以在設定網站名稱時使用變數。 如需詳細資訊，請檢視[使用變數來設定目的地路徑、網站名稱或檔案名稱選項](./generate-output-use-variables.md) |
...
| 根據以下條件產生頁面名稱： | **主題檔案名稱**：使用主題的檔案名稱來建立網站URL。<br> **主題標題**：使用主題的標題來建立Experience Manager網站名稱。<br> **注意**：此選項只有在為您的環境啟用時才能使用。 請聯絡Adobe客戶成功團隊以啟用它。 當可用時，在此預設集中選取的值會覆寫資料夾層級`aemsite.pagetitle`設定。 如需詳細資訊，請檢視[自訂現有的AEM網站輸出](../install-conf-guide/customize-aem-site-output.md) |
| 為從地圖中移除的主題刪除先前產生的頁面 | 如果DTIA對映的結構變更，您可以使用此選項來移除先前為已移除主題產生的頁面。 此功能僅適用於完整地圖發佈。<br><br>假設您已發佈DITA map，其中包含主題a.dita、b.dita和c.dita。 再次發佈對應之前，您已從對應移除b.dita主題。 現在，如果您已選取此選項，則會從AEM Sites輸出中移除與b.dita相關的所有內容，並且僅發佈a.dita和c.dita。<br><br>**注意**：有關已刪除頁面的資訊也會擷取到輸出產生記錄中。 如需有關存取記錄檔的詳細資訊，請[檢視並檢查記錄檔](generate-output-basic-troubleshooting.md#id1821I0Y0G0A__id1822G0P0CHS)。 <br><br>**警告**：刪除主題時，已發佈網站的頁面會變成無法使用。 因此，在刪除主題之前，會顯示警告。 您必須確認以刪除它們。 |
...
![新](images/aem-sites-content-tab.png){width="650"}
