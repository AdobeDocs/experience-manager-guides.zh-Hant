...
- [來源](#source-view)
- [預覽](#preview)
- [並排檢視](#side-by-side-view-available-in-new-editor-only)
...
**作者**&#x200B;檢視可讓您在編輯器中編輯您的DITA map。 這會顯示地圖編輯器的WYSIWYG檢視。
...
**功能表**&#x200B;下拉式清單中的可用選項與[編輯器工具列](./web-editor-toolbar.md#menu-dropdown)中的可用選項相同。

此外，您可以檢視下列圖示，並從「作者」檢視執行相關工作：

**插入在前/插入在後** - ![](images/insert_element_before_icon.svg) / ![](images/insert_element_after_icon.svg)

顯示&#x200B;**在**&#x200B;之前插入專案或&#x200B;**在**&#x200B;之後插入專案。 選取您要插入對映中的元素。 視作業而定，新元素會插入到對映中目前元素之前或之後。
...
>[!NOTE]
>
> 在新編輯器中，編輯器設定下提供新增或移除&#x200B;**標籤**&#x200B;的選項。 如需詳細資訊，請檢視[編輯器設定](./config-editor-settings.md)。
...
>[!NOTE]
>
> 本節僅適用於舊編輯器。 在新編輯器中，地圖及其主題會自動重新整理。
...
「新編輯器」中的「並排」檢視可讓您檢視彼此相鄰的「作者」和「Source」檢視。 這可讓您更輕鬆地瞭解您如何建置對應，並確保您所做的任何變更都與預期的結構一致。

例如，如果主題未正確顯示在輸出中，您可以使用作者檢視來檢查其在地圖中的位置，同時檢閱Source檢視來檢閱主題參考或屬性。
...
1. 瀏覽至Explorer並開啟您要編輯的對映檔案。

   對應檔案會在「對應」檢視中開啟。

   >[!NOTE]
   >
   > 您也可以使用Assets UI來開啟對應檔案。 導覽至包含您要編輯之主題的DITA map檔案，並在主工具列中選取&#x200B;**編輯主題**&#x200B;以啟動編輯器。

1. 選取任何主題，在編輯器中開啟主題以進行編輯。
...
右側面板會顯示「地圖編輯器」的「版面」檢視中的「內容」屬性和「地圖」屬性。

**內容屬性**

「內容」屬性面板包含對映中目前選取之主題的相關資訊，包括其型別、連結路徑、連結UUID和屬性。

如需詳細資訊，請在編輯器中檢視[內容屬性](web-editor-features.md#right-panel)。
...
![配置檢視](images/layout-inline-attributes.png){width="650"}

請參閱下列標籤，比較屬性在新編輯器和舊編輯器的製作檢視中的顯示方式。

>[!BEGINTABS]

>[!TAB 新編輯器]

![作者檢視](images/author-inline-attributes-editor-2-0.png){width="650"}

>[!TAB 舊編輯器]

![作者檢視](images/author-inline-attributes.png){width="650"}
