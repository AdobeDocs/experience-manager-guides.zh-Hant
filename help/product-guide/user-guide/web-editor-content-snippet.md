...
選取 **資料來源** ![資料來源](images/data-source-icon.svg) 以檢視連線的資料來源。 「資料來源」面板隨即開啟並顯示所有連線的資料來源。
...
1. 在 **資料查詢** 文字方塊。  選取 **複製範例查詢** 以快速複製資料查詢。 您可以將範例查詢複製並貼到中，而不是手動建立查詢 **資料查詢** 文字方塊。 之後，只要根據您的資料需求編輯查詢即可。
...
1. 此 **資源** 某些聯結器會顯示下拉式清單，例如REST Client、Salsify、Akeneo和Microsoft ADO。  從下拉式清單中選取任何資源，並連線至該資源，以使用產生器建立內容片段或主題。
