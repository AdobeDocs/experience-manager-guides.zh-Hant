...
將您的引文插入主題後，您可以在網頁編輯器中預覽它們。 您也可以使用原生PDF發佈引文內容。

![引用列在面板中](assets/citation-panel.png){width="300"}
...
內容片段是AEM中的分散式內容片段。 它們是以內容模型為基礎的結構化內容。 內容片段是純內容，沒有設計或版面配置資訊。 可獨立於AEM支援的管道之外獨立編寫和管理這些管道。 內容片段的模組化和可重複使用性可帶來更大的彈性、一致性、效率，以及更簡單的管理。
...
![發佈內容片段的選項](assets/content-fragment-publish.png){width="550"}
...
如需更多詳細資料，請檢視[將浮水印新增至草稿檔案的PDF輸出](../native-pdf/use-javascript-content-style.md#watermark-draft-document)。
...
![新增原生pdf的中繼資料](assets/native-pdf-metadata-asset.png){width="300"}
...
依預設，在原生PDF輸出中，所有區段都會從新頁面開始。 現在，您可以將區段合併至其上一頁或下一頁。 這會以在PDF輸出中選取的頁面發佈連續區段，且中間沒有分頁符號。

如需更多詳細資料，請在[頁面順序](../native-pdf/components-pdf-template.md#page-order)區段中檢視合併頁面功能說明。
...
![原生pdf的進階設定對話方塊](assets/native-pdf-advanced-settings.png){width="800"}
...
![在原生pdf範本中新增樣式](assets/add-styles-native-pdf.png){width="300"}
...
![檔案的選項功能表](assets/rename-move-assets.png){width="550"}
...
![中斷連結報告](assets/broken-link-report.png){width="800"}
...
使用此格式，您可以將內容匯出為業界標準XLIFF格式，然後提供相同的內容給翻譯廠商。如需詳細資訊，請檢視[建立翻譯專案](../user-guide/translate-documents-web-editor.md#create-translation-project)。

![翻譯專案型別](assets/translation-project-types.png){width="350"}
...
* 現在，您不僅可以選取個別預設集，還可以一次為DITA map啟用所有資料夾設定檔預設集。
  ![編輯地圖集合](assets/edit-map-collection.png){width="800"}
...
![我的最愛面板](assets/favorites-options.png){width="650"}
...
![使用者偏好設定](assets/device-theme-user-preferences.png){width="550"}
