...
description: 瞭解相容性矩陣，以及如何升級至Adobe Experience Manager Guides as a Cloud Service 2024.04.0版。
exl-id: deca46e5-12cc-497f-84af-61ee02da3d65
TQID: https://experienceleague.adobe.com/w-nncERMrsiWN7dTG9fJiGuDk8ynLkrikmKNlKoonQk
...
本節列出2024.04.0版Experience Manager Guides as a Cloud Service支援的軟體應用程式相容性矩陣。
...
1. 對伺服器執行POST要求（使用正確的驗證） - `http://<server:port>/bin/guides/map-find/indexing`。 （選用：您可以傳遞地圖的特定路徑來為其編制索引，依預設所有地圖都會編制索引||例如： `https://<Server:port>/bin/guides/map-find/indexing?paths=<path of the MAP in repository>`）
