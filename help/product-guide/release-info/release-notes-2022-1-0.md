...
1. 將Cloud Services Git程式碼的`/dox/dox.installer/pom.xml`檔案中的`<dox.version>`屬性更新為2022.1.78。
...
* 即使重新整理頁面後，新建立的地圖集合也不會列出。(8603)
* 無法關閉已解鎖的主題。 (8545)
* 在來源和作者模式之間切換會將主題標籤為已變更，並需要再次儲存內容。(8524)
* 搜尋特殊字元`[`或`*`時，重複使用內容面板當機。(8279)
* 使用鍵盤快速鍵Alt+Enter開啟插入元素對話方塊時，搜尋列中不會顯示游標。(7912)
