...
Click the left or right arrow icons&#39; to move the topic left or right. 以此方式移動主題會使它成為與其上主題相關的子項\（巢狀）或同層級\（移除巢狀\）。

**B：屬性**

按一下「屬性」圖示以開啟「Topicref屬性」對話方塊。 您可以使用此對話方塊來設定主題屬性和中繼資料資訊。 如需有關標準主題屬性和中繼資料的詳細資訊，請參閱OASIS DITA語言規格中的[topicref](https://docs.oasis-open.org/dita/v1.2/os/spec/langref/topicref.html)檔案。
...
按一下「金鑰」圖示以新增金鑰定義。 任何被覆寫的鍵或已在地圖中定義的鍵都會以紅色顯示。 If you click the Properties icon on a key definition, you get the Keydef Properties dialog.
