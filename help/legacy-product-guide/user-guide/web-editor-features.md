...
You can resize an image by providing either Height or Width value for the image file. The aspect ratio of the image is maintained automatically. 如果需要，您也可以按一下鎖定圖示\(屬於「維持外觀比例」(Maintain Aspect Ratio)\)並提供「高度」(Height)和「寬度」(Width)值，選擇不維持影像檔案的外觀比例。
...
>[!NOTE]
>
> 如果使用者從網頁編輯器啟用「標籤檢視」，則即使跨工作階段，該檢視也會保持啟用狀態。 這表示您不需要再次啟用標籤檢視即可稍後存取。新使用者作業階段的「標籤檢視」預設值由ui\_config.json檔案中的tagsView屬性決定。 如需詳細資訊，請參閱安裝和設定Adobe Experience Manager Guides as a Cloud Service中的&#x200B;*設定標籤檢視的預設值*&#x200B;區段。
...
1. 您也可以在&#x200B;**版本記錄**&#x200B;對話方塊中檢視下列資訊：
   ...
   **中繼資料**標籤：新加入的中繼資料為綠色字型，而刪除的中繼資料為紅色字型。
   ![版本的中繼資料差異 ](images/metadata-version-diff.png){width="550" align="left"}
   *比較版本記錄中不同版本的中繼資料。*
...
您也可以在Web編輯器中按一下並播放音訊或視訊檔案。 You can change the volume or
...
- **Add To**: You can choose from the following options:
   - **Favorites**: Adds the selected file to favorites. 您可以選擇將它新增至現有或新的「我的最愛」集合。

   - **Reusable Contents**: Adds the selected file to the Reusable Contents list in the left panel.

- **Properties**: Use this to open the properties page of the selected file. 也可以選取檔案並按一下工具列中的「屬性」圖示，從Assets UI存取此屬性頁面。
...
- 按一下「大綱檢視」中的元素，可選取元素在「作者」或「Source」檢視中的內容。「大綱」檢視會與「作者」檢視和「Source」檢視保持同步。 如果您在任何檢視中進行任何變更，可以在「輪廓」檢視中看到它們。 例如，如果您在「作者」檢視中新增段落或更新元素，它會顯示在「大綱」檢視中。
...
- 按一下「代碼片段」旁的+圖示，開啟「新增代碼片段」對話方塊。
  ...
  In the New Snippet dialog, provide a title which appears in the Snippets panel, a description, and XML code of the snippet content that you want to create. 按一下&#x200B;**建立**&#x200B;以儲存並建立程式碼片段。
...
1. 您也可以以下列方式篩選專案：
   ...
   - 選取![](images/filter-search-icon.svg)以開啟&#x200B;**篩選器**&#x200B;對話方塊。 您可以選取所有專案或僅選取特定專案。 The selected projects are listed in the **Review** panel.
     ![](images/active-review-select-project.png){width="300" align="left"}

     依預設會啟用由我起始的&#x200B;**工作**&#x200B;選項。 It allows you to view only the tasks that you&#39;ve initiated. 即使在重新整理頁面後，也會維持此選項的切換狀態。
...
>[!NOTE]
>
> As an administrator, you can also choose to view the list of filenames in the Web Editor. 選取&#x200B;**使用者偏好設定** ![](images/user_preference_editor_icon.svg)中&#x200B;**依**&#x200B;檢視檔案節的&#x200B;**檔案名稱**&#x200B;選項。
