...
- **發佈設定檔**：這包含可用來發佈&#x200B;**知識庫**&#x200B;輸出的發佈設定檔。 您可以為目標知識庫建立新的設定檔。 例如，Salesforce或ServiceNow。
  ...
  這些可用來建立Salesforce發佈設定檔。
   ...
   - 若要建立Salesforce發佈設定檔，請從&#x200B;**伺服器型別**&#x200B;下拉式清單中選取&#x200B;**Salesforce**&#x200B;知識庫。輸入設定檔名稱。在&#x200B;**網站URL**&#x200B;中，輸入您要用來發佈輸出的消費者網站，然後新增Salesforce消費者網站提供的&#x200B;**消費者金鑰**&#x200B;和&#x200B;**消費者機密**。然後，**驗證**&#x200B;並&#x200B;**儲存**&#x200B;新建立的設定檔。
     在編輯器設定中![salesforce發佈設定檔](./images/salesforce-publish-profile.png){width="550"}
...
- **驗證**：此標籤包含在Web編輯器中設定Schematron驗證的選項。 您可以啟用下列功能：
   ...
   - **在儲存檔案之前執行驗證檢查**：選取此項以在任何儲存作業之前使用選取的Schematron檔案執行Schematron驗證。 您可以按一下+圖示來新增Schematron檔案。 隨即列出選取的Schematron檔案。
     ...
     在編輯器設定中進行![驗證](./images/editor-setting-validation.png){width="550"}
...
- **中繼資料**：您可以控制主題的版本中繼資料及其值顯示在&#x200B;**版本記錄**&#x200B;對話方塊中。  在中繼資料路徑中，指定您要從中挑選中繼資料的節點位置。 您也可以為中繼資料定義自訂名稱作為標籤。 預設屬性為「標題」、「檔案狀態」和「標籤」。
  ...
  編輯器設定中的![中繼資料標籤](images/editor-setting-metadata.png){width="550"}
...
- **一般**：「一般」標籤可讓您設定下列設定：
  ...
  ![使用者偏好設定的一般標籤](images/user_preference_editor.PNG){width="550"}
...
- **外觀**：選取網頁編輯器應用程式的主題和內容編輯區域的來源檢視。
  ...
  使用者偏好設定的![外觀標籤](images/user_preference_editor_appearance.png){width="550"}
...
![插入專案](images/insert-element.png){width="300"}

*輸入&#39;t&#39;以搜尋以&#39;t&#39;開頭的所有有效元素。*

如果您在區塊元素（如`note`）內工作，則使用[插入元素]圖示在`note`元素之後插入新元素。 在下列熒幕擷圖中，筆記元素已插入p \(paragraph\)元素內：

![在區塊元素中插入元素](images/note-in-para-insert-element_cs.png){width="800"}
...
1. 按一下&#x200B;**版本記錄**。
   ...
   **版本記錄**&#x200B;對話方塊就會顯示。
   ...
   ![版本記錄對話方塊](images/version-history-dialog-web-editor.png){width="550"}
   *預覽不同版本主題中的變更。*
...
1. 您也可以在&#x200B;**版本記錄**&#x200B;對話方塊中檢視下列資訊：
   ...
   **中繼資料**&#x200B;標籤：新加入的中繼資料為綠色字型，而刪除的中繼資料為紅色字型。
   ![版本](images/metadata-version-diff.png){width="550"}的中繼資料差異
   *比較版本記錄中不同版本的中繼資料。*
...
![進階篩選器對話方塊](images/repository-filter-search-advanced.png) {width="800"}
...
1. 在左側面板上選取檢閱![](images/active-review-tasklist-icon.svg)。 **檢閱**&#x200B;面板隨即開啟。  系統會顯示您所屬之審閱專案中的所有審閱專案與作用中審閱任務。
...
1. 若要編輯屬性，請將滑鼠停留在它上並選取&#x200B;**編輯** ![編輯圖示](images/edit_pencil_icon.svg)。
   ![編輯屬性](images/edit-attributes-content-properties.png){width="300"}
